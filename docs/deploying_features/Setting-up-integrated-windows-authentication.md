...
Note, that we recommend that you use a strong authentication mechanism such as OpenID Connect, where you have the ability to enforce multi-factor authentication on users attempting to access your application. Access Manager fully supports modern OIDC providers such as [Azure AD](/setting_up_authentication/Setting-up-authentication-with-Azure-AD) and [Okta](/setting_up_authentication/Setting-up-authentication-with-Okta). 
