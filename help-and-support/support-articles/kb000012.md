...
### 1. Incorrect SPN
The hostname that the user types into their address bar must be registered as an SPN on the service account in Active Directory.
The SPN must be in the format of `HTTP/hostname` where `hostname` is the fully qualified host name that the user is entering into their browser.
